...
class condGANTrainer(object):
    def __init__(self, args,output_dir, data_loader, n_words, ixtoword):
        self.args=args
        if cfg.TRAIN.FLAG:
    ...
    def save_img_results(self, netG, noise, sent_emb, words_embs, mask,image_encoder, captions, cap_lens,gen_iterations, name='current'):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                errG_total.backward()
                optimizerG.step()
                for p, avg_p in zip(netG.parameters(), avg_param_G):#滑动平均
                ...
                if gen_iterations % 1000 == 0:
                    backup_para = copy_G_params(netG)
                    load_params(netG, avg_param_G)
                    self.save_img_results(netG, fixed_noise, sent_emb,words_embs, mask, image_encoder,captions, cap_lens, epoch, name='average')
                    load_params(netG, backup_para)
            end_t = time.time()

            print('''[%d/%d][%d] Loss_D: %.2f Loss_G: %.2f Time: %.2fs''' % (epoch, self.max_epoch, self.num_batches, errD_total.data[0], errG_total.data[0], end_t - start_t))
                ...
                for i in range(1):  # 16
                    ...
                    for j in range(batch_size):
                        ...
                        for k in range(len(attention_maps)):
                            ...
                            att_sze = attn_maps.size(2)
                            img_set, sentences = build_super_images2(im[j].unsqueeze(0),captions[j].unsqueeze(0),[cap_lens_np[j]], self.ixtoword,[attn_maps[j]], att_sze)
                            if img_set is not None:
                                im = Image.fromarray(img_set)
